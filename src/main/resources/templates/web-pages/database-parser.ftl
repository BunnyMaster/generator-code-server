    ...
    <el-row class="w-[1200px] m-auto p-4 flex flex-col">
        ...
        <el-tabs type="border-card" v-model="activeTab" @tab-change="onTabChange">
            ...
            <el-tab-pane label="通用配置" name="common">
                <common-config v-model:form="configForm" v-model:provider-from="providerFrom"
                               ref="databaseConfigRef">
                    <!-- 指定生成的表名称 -->
                    <el-form-item label="指定生成的表名称" prop="tableNames">
                        <div class="dynamic-tags">
                            <el-tag class="mr-2 mb-2" v-for="(table, index) in configForm.extra.tableNames" :key="index"
                                    @close="removeTableName(index)" closable>
                                {{ table }}
                            </el-tag>
                        </div>
                        <el-input v-model="newTableName" class="tag-input" clearable placeholder="请输入表名"
                                  @keyup.enter="addTableName">
                            <template #append>
                                <el-button @click="addTableName">添加</el-button>
                            </template>
                        </el-input>
                        <el-text type="info">提示：按回车键或点击添加按钮添加表名</el-text>
                    </el-form-item>

                    <!-- 当前数据库 -->
                    <el-form-item label="当前数据库" prop="database" class="w-[220px]">
                        <el-input v-model="configForm.extra.database" placeholder="请输入数据库名称" disabled/>
                    </el-form-item>
                </common-config>
            </el-tab-pane>
        ...
        <!-- 配置预览 -->
        <config-preview :form="configForm" :generation-strategy-from="providerFrom" :generator-views="generatorViews"/>
</div>
</body>
...
<script>
    ...
    const app = createApp({
        setup() {
            ...
            // 支持哪些策略
            const providerFrom = reactive({provider: "MySQL", mode: "print"});
             ...
             */
            const addTableName = () => {
                if (newTableName.value && !configForm.extra.tableNames.includes(newTableName.value)) {
                    configForm.extra.tableNames.push(newTableName.value);
                    newTableName.value = '';
                    return;
                }

                message({message: '表名未填写或表名已经存在', type: 'warning'});
            };

            /**
             * 根据索引移除一个表名
             */
            const removeTableName = (index) => {
                configForm.extra.tableNames.splice(index, 1);
            };
            ...
            const loadDefaultConfig = async () => {
                try {
                    ...
                    configForm.extra.tableNames = ["sys_dept", "sys_dict"]
